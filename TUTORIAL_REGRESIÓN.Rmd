...
Se observa que el p-valor es mucho menor que el alpha, por lo tanto, se rechaza H0, entonces podemos concluir que al menos una de las variables tienen un efecto significativo sobre la variable respuesta (nota_despues).
...
Se crea un nuevo data.frame que representa el perfil promedio definido anteriormente.
...
Se utiliza el modelo ajustado para obtener la estimación puntual de la nota final promedio correspondiente a este perfil.
...
Para cuantificar la incertidumbre asociada a la estimación de la media, se construye un intervalo de confianza al 95%.
...
l.1 Predicción puntual

Paso 2: Construcción del nuevo conjunto de datos (estudiante individual)

Se define un nuevo data.frame que representa al estudiante individual a predecir.
...
Se obtiene la predicción puntual de la nota final del estudiante utilizando el modelo ajustado.
...
Utilizando el modelo de regresión lineal múltiple ajustado, se predice que la nota final esperada para un estudiante que estudia 4 horas diarias, utiliza herramientas de inteligencia artificial y tenía una nota previa de 70 puntos es aproximadamente 79.70 puntos.

l.2 Predicción intervalar (Intervalo de Predicción)
