...
```{r}
# Resumen del modelo para pruebas globales
anova(modelo)
```
...
#### Pruebas de hipótesis individuales (significancia de los coeficientes)

Una vez verificada la significancia global del modelo, se procede a evaluar la significancia individual de cada coeficiente de regresión mediante la prueba *t* de Student. Estas pruebas permiten determinar si cada variable explicativa tiene un efecto estadísticamente significativo sobre la calificación posterior al uso de IA, manteniendo constantes las demás variables del modelo.

Las hipótesis planteadas para cada coeficiente $\beta_j$ son:
...
```{r}
# Resumen del modelo para pruebas individuales
summary(modelo)
...
- **Horas de estudio por día (`horas_estudio`)**:  
  El coeficiente asociado no resulta estadísticamente significativo (valor p > 0.05), lo que indica que, controlando por el uso de IA y la calificación previa, las horas de estudio no presentan un efecto significativo sobre la calificación final.

- **Uso de inteligencia artificial (`usa_ia`)**:  
  El coeficiente asociado a esta variable es estadísticamente significativo (valor p < 0.05) y positivo, lo que sugiere que los estudiantes que utilizan herramientas de inteligencia artificial tienden a obtener, en promedio, una calificación final mayor que aquellos que no las utilizan, manteniendo constantes las demás variables.

- **Calificación previa (`nota_antes`)**:  
  El coeficiente correspondiente es estadísticamente significativo (valor p < 0.05) y positivo, indicando que un mayor rendimiento académico previo se asocia con una mayor calificación posterior.

**En conjunto, las pruebas individuales confirman que el uso de IA y la calificación previa son variables relevantes para explicar el rendimiento académico posterior, mientras que las horas de estudio no muestran un efecto significativo en este modelo específico.**

**Nota de precaución:** Las pruebas de significancia individuales se interpretan bajo el supuesto de cumplimiento de los supuestos clásicos del modelo de regresión lineal. En caso de desviaciones, los resultados deben analizarse con cautela, especialmente en lo referente a la inferencia estadística.
